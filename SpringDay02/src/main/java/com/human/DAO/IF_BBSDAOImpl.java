...
public class IF_BBSDAOImpl implements IF_BBSDAO {
	// 서비스단에서 호출한 작업을 sqlsession을 통해서 실제 데이터 베이스 작업을 실행하기 위한 DAO
	
	@Inject // 컨테이너로부터 SqlSession 타입 객체를 주입 받아라.. 그러면 sqlSession참조변수 객체를 주입받아 참조 가능
	private SqlSession sqlSession; // mybatis에서 온 객체
